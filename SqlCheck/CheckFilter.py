...
user_id = []
a = 1
fileter_list=[805493,810589,813439,805621,805827,805505,808806,806011,810717,805715,815871,810909,806645,809957,810991,811825,806212,814451,805617,812815,816492,808417,807074,806005,806580,810589,813535,814430,807673,824759,820411,814424,826132,814150]
...
def getID(id,b):
    ...
    for i in id:
        for f in i:
            ...
            for g in some_id:
                print(g[0])
                if g[0] in fileter_list:
                    print('重复的ID为：'+str(g[0]))
                    continue
                else:
                    data.append(g)
    user_id.append(data)
...
def SaveExcle():
    ...
    for i in user_id:
        for g in i:
            for z in g:
                cursor.execute("SELECT a.id,i.uid,a.mobile,a.referenceId,i.balance,i.free_integral,i.holdlock_integral,i.lock_integral,i.withdraw_integral,i.integral_type,i.create_time FROM useraccount a LEFT JOIN user_integral i ON a.id=i.uid WHERE a.id=%r and i.integral_type='2';" %z)
